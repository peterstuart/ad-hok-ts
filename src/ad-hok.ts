...
const branch = <A, B>(condition: (value: B) => boolean, left: A) =>
  flatMapEither((value: B) =>
    condition(value) ? newLeft(left) : newRight(value)
  );
...
const returns = <A, B, C>(f: (props: B) => C) =>
  flatMapEither((props: B) => newLeft(f(props)));

const flow2 = <A, B, C>(f1: (a: A) => B, f2: (b: B) => C) => (a: A) =>
  f2(f1(a));
const flow3 = <A, B, C, D>(
  f1: (a: A) => B,
  f2: (b: B) => C,
